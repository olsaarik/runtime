...
namespace System.Text.RegularExpressions.Symbolic
{
    ...
    internal sealed partial class SymbolicRegexMatcher<TSet> : SymbolicRegexMatcher where TSet : IComparable<TSet>, IEquatable<TSet>
    {
        ...
        public SymbolicMatch FindMatch(RegexRunnerMode mode, ReadOnlySpan<char> input, int startat, PerThreadData perThreadData)
        {
            ...
            {
                var (captureStarts, captureEnds) = FindSubcaptures(input, matchStart, matchEnd, perThreadData);
                return new SymbolicMatch(matchStart, matchEnd - matchStart, captureStarts, captureEnds);
            }
        }
        ...
        private (ArraySegment<int> CaptureStarts, ArraySegment<int> CaptureEnds) FindSubcaptures(ReadOnlySpan<char> input, int i, int iEnd, PerThreadData perThreadData)
        {
            ...
            int capsize = _capsize;
            Registers initialRegisters = perThreadData.AcquireRegisters(capsize);
            initialRegisters.StateReferenceCount = 1;
            ...
            while ((uint)i < (uint)iEnd)
            {
                ...
                uint charKind = GetCharKind(input, i);

                int k = 0;
                for (; k < current.Count; ++k)
                {
                    (int sourceId, Registers sourceRegisters) = current.Values[k];
                    ...
                    List<(DfaMatchingState<TSet>, DerivativeEffect[])> transitions =
                        ...
                        CreateNewCapturingTransitions(sourceState, builder.GetMinterm(mintermId), offset);

                    if (transitions.Count == 0)
                    {
                        sourceRegisters.StateReferenceCount -= 1;
                    }
                    ...
                    for (int j = 0; j < transitions.Count; ++j)
                    {
                        ...
                        {
                            Registers newRegisters = sourceRegisters.WithEffects(effects, i, capsize, perThreadData);
                            newRegisters.StateReferenceCount += 1;
                            next.Update(index, targetState.Id, newRegisters);
                        }
                    }

                    if (sourceRegisters.StateReferenceCount == 0)
                    {
                        perThreadData.ReleaseRegisters(sourceRegisters);
                    }

                    if (sourceState.IsNullableFor(charKind))
                    {
                ...
                for (; k < current.Count; ++k)
                {
                    (int _, Registers sourceRegisters) = current.Values[k];
                    sourceRegisters.StateReferenceCount -= 1;
                    if (sourceRegisters.StateReferenceCount == 0)
                    {
                        perThreadData.ReleaseRegisters(sourceRegisters);
            ...
            try
            {
                foreach (var (endStateId, endRegisters) in current.Values)
                {
                    ...
                    if (endState.IsNullableFor(GetCharKind(input, iEnd)))
                    {
                        ...
                        endState.Node.ApplyEffects((effect, args) => args.Registers.ApplyEffect(effect, args.Pos, _capsize),
                            CharKind.Context(endState.PrevCharKind, GetCharKind(input, iEnd)), (Registers: endRegisters, Pos: iEnd));
                        return (endRegisters.GetCaptureStarts(capsize), endRegisters.GetCaptureEnds(capsize));
                    }
                }
            }
            finally
            {
                foreach (var (_, endRegisters) in current.Values)
                {
                    endRegisters.StateReferenceCount -= 1;
                    if (endRegisters.StateReferenceCount == 0)
                    {
                        perThreadData.ReleaseRegisters(endRegisters);
                    }
                }
            }
        ...
        internal struct Registers
        {
            ...
            public Registers(int capsize)
            {
                Values = new int[1 + capsize * 2];
            }

            public int[] Values { get; set; }

            public ref int StateReferenceCount => ref Values[0];
            ...
            /// <param name="capsize">the number of captures in the related pattern</param>
            /// <param name="perThreadData"></param>
            public Registers WithEffects(DerivativeEffect[] effects, int pos, int capsize, PerThreadData perThreadData)
            {
                if (effects.Length == 0)
                    return this;
                Registers result = StateReferenceCount == 0 ? this : perThreadData.CloneRegisters(this);
                foreach (DerivativeEffect effect in effects)
                {
                    result.ApplyEffect(effect, pos, capsize);
                }
                return result;
            }
            ...
            /// <param name="pos">the current input position to record</param>
            /// <param name="capsize">the number of captures in the related pattern</param>
            public void ApplyEffect(DerivativeEffect effect, int pos, int capsize)
            {
                switch (effect.Kind)
                {
                    case DerivativeEffectKind.CaptureStart:
                        Values[1 + effect.CaptureNumber] = pos + 1;
                        break;
                    case DerivativeEffectKind.CaptureEnd:
                        Values[1 + capsize + effect.CaptureNumber] = pos + 1;
                        break;
                }
            }

            public void Reset() => Array.Fill(Values, 0);

            public ArraySegment<int> GetCaptureStarts(int numCaptures) => new ArraySegment<int>(Values, 1, numCaptures);

            public ArraySegment<int> GetCaptureEnds(int numCaptures) => new ArraySegment<int>(Values, 1 + numCaptures, numCaptures);
        }
        ...
        internal sealed class PerThreadData
        {
            ...
            public readonly Stack<Registers>? RegistersCache;
            ...
            public PerThreadData(SymbolicRegexBuilder<TSet> builder, int capsize)
            {
                ...
                if (capsize > 1)
                {
                    ...
                    RegistersCache = new Stack<Registers>();
                }
            }

            public Registers AcquireRegisters(int capsize)
            {
                if (RegistersCache!.TryPop(out Registers result))
                {
                    result.Reset();
                }
                else
                {
                    result = new Registers(capsize);
                }
                return result;
            }
            ...
            public Registers CloneRegisters(Registers registers)
            {
                if (RegistersCache!.TryPop(out Registers result))
                {
                    Array.Copy(registers.Values, result.Values, result.Values.Length);
                }
                else
                {
                    result = new Registers((int[])registers.Values.Clone());
                }
                result.StateReferenceCount = 0;
                return result;
            }

            public void ReleaseRegisters(Registers registers) => RegistersCache!.Push(registers);
        }
